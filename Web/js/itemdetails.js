...
function inpucart() {
    $(".single_top").on("click", "#inputcart", function () {
        ...
        $.ajax({
            ...
            dataType: "json",
             url: "http://192.168.199.242:8080/BSMD/car/addToCar.do",
...
function collect() {
    $(".btnform").on("click", ".cfont", function (e) {
        ...
        if(username!=null&&username!=""){
            ...
            $.ajax({
                type: "post",
                 url: "http://192.168.199.242:8080/BSMD/insertCollection.do",
...
$.loadCom = function () {
    var reg = getUrlParam("id");
    var info = {"itemno": "10001", "pageindex": "1", "pagecount": "10"};
    $.ajax({
        type: "post",
...
$.loadDetail = function (reg, pos) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            html += '<li><a href="itemlist.html?bigclass=' + bigclass + '&pos=' + pos + '">' + bigclass + '</a></li>';
            var subd = json.detail.propertyName;
            html += '<li><a href="itemlist.html?bigclass=' + bigclass + '&propertyName=' + subd + '&pos=' + pos + '">' + subd + '</a></li>';
            html += '<li class="active">' + json.detail.itemName + '</li></ol></div>';
            //面包屑end
            html += '<div class="single_right"><div class="detail_left"><div class="images_3_of_2"><ul id="etalage">';

            //修改，data-获取数量的也有修改
            var lg = json.imageTop.length;
            for (var i = 0; i < lg; i++) {
                html += '<li><a href="#"><img class="etalage_thumb_image"src="' + json.imageTop[i] + '"class="img-responsive"/>';
                //html += '<img class="etalage_source_image" src="' + json.imageTop[i]  + '"class="img-responsive" title="商品详情"/></a></li>';
                html += '<img class="etalage_source_image" src="' + json.imageTop[i] + '" class="img-responsive"' + '/></a></li>';
            }
            //
            html += '</ul></div></div>';
